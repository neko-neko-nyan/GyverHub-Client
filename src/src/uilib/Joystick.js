...
function adjust(color, ratio) {
  return '#' + color.replace(/^#/, '').replace(/../g, color =>
      ('0' + Math.min(255, Math.max(0, Math.floor(parseInt(color, 16) * (ratio + 1)))).toString(16)).slice(-2));
}
...
export default class Joystick {
  ...
  constructor(cont, dpad, color, auto, exp, callback) {
    ...
    if (!this.cv || !this.cv.parentElement.clientWidth) return;
    this.size = this.cv.parentElement.clientWidth;
